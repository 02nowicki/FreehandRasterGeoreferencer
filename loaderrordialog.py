...
# Opis / Description    : Migracja Qt5 → Qt6: done(QDialog.accepted) →
#                         done(_dialog_accepted_value())
# =============================================================================

import os.path

from qgis.PyQt.QtCore import qDebug, Qt
...
class LoadErrorDialog(QDialog, Ui_LoadError):
    ...
    def accept(self):
        ...
        if result:
            self.done(_dialog_accepted_value())
        else:
            msgBox = QMessageBox()
