...
"""
# =============================================================================
# ZMIANY / CHANGES - wersja 0.9 / version 0.9
# Autor / Author        : 02nowicki - Paweł Nowicki
# Data zmian / Date     : 2026-05-08
# Kompatybilność / Compat: QGIS 4.0.2 (Qt6 / PyQt6)
# Opis / Description    : Migracja Qt5 → Qt6: QDialog.Accepted, tryb mieszania
#                         (blend mode) jako enum PyQt6, helper _dialog_accepted_value()
# =============================================================================
...
def _dialog_accepted_value():
    ...
    return getattr(QDialog, "Accepted", 1)


def _composition_mode_by_name(modeName):
    attr_name = "CompositionMode_" + modeName

    # Qt5 style: QPainter.CompositionMode_SourceOver
    mode = getattr(QPainter, attr_name, None)
    if mode is not None:
        return mode

    # Qt6 style: QPainter.CompositionMode.CompositionMode_SourceOver
    composition_enum = getattr(QPainter, "CompositionMode", None)
    if composition_enum is not None and hasattr(composition_enum, attr_name):
        return getattr(composition_enum, attr_name)

    # Safe fallback to SourceOver if modeName is unknown
    if composition_enum is not None and hasattr(
        composition_enum, "CompositionMode_SourceOver"
    ):
        return composition_enum.CompositionMode_SourceOver
    return getattr(QPainter, "CompositionMode_SourceOver", None)
...
class FreehandRasterGeoreferencerLayer(QgsPluginLayer):
    ...
    def initializeLayer(self, screenExtent=None):
        ...
        if self.filepath is not None:
            ...
            if not os.path.exists(filepath):
                ...
                if result == _dialog_accepted_value():
                    # absolute
    ...
    def setBlendModeByName(self, modeName):
        ...
        blendMode = _composition_mode_by_name(modeName)
