...
# Opis / Description    : Migracja Qt5 → Qt6: porównania przycisków myszy
#                         z int na Qt.MouseButton enum, dodano import Qgis
# =============================================================================
...
def _mouse_button(name, fallback):
    mouse_button = getattr(Qt, "MouseButton", Qt)
    return getattr(mouse_button, name, fallback)


LEFT_BUTTON = _mouse_button("LeftButton", 1)
RIGHT_BUTTON = _mouse_button("RightButton", 2)
...
class ScaleRasterMapTool(QgsMapToolEmitPoint):
    ...
    def canvasReleaseEvent(self, e):
        ...
        if pressed_button == LEFT_BUTTON:
            ...
            setLayerVisible(self.iface, self.layer, self.isLayerVisible)
        elif pressed_button == RIGHT_BUTTON:
...
class GeorefRasterBy2PointsMapTool(QgsMapToolEmitPoint):
    def __init__(self, iface):
        ...
        self.rubberBandOrigin.setIcon(QgsRubberBand.IconType.ICON_CIRCLE)
